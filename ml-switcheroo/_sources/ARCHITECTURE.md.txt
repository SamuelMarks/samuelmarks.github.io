...
graph TD
    ...
    classDef optimization fill:#ea4335,stroke:#20344b,stroke-width:2px,color:#ffffff,font-family:'Google Sans',rx:5px;
    ...
    subgraph ENGINE ["AST Engine"]
        ...
        REWRITER("3. Pivot Rewriter"):::process
...
### 3. Rewriting Phase (`PivotRewriter`)

The core transformer is built on a Mixin architecture:

* **StructureMixin:** Handles Class/Function definitions (swaps Base classes, renames `forward`, injects `rngs` args).
* **CallMixin:** The workhorse. Resolves calls to Abstract IDs, applies Layout Permutation, Argument Normalization, and
  dispatches to Plugins.
* **AttributeMixin:** Handles constant renaming (e.g., `torch.float32` $\to$ `jnp.float32`) and stateful assignment
  tracking.
